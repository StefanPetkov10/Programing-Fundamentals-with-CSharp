...
namespace _01._Advertisement_Message
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                string output = phrases[generator.Next(0, phrases.Count)] + " " +
                       events[generator.Next(0, events.Count)] + " " +
                      authors[generator.Next(0, authors.Count)] + $" - " +
                     cities[generator.Next(0, cities.Count)];
